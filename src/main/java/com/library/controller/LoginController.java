...
public class LoginController {
    ...
    public @ResponseBody Object loginCheck(@RequestBody(required = false) java.util.Map<String, Object> body,
            HttpServletRequest request) {
        String username = null;
        String passwd = null;
        if (body != null) {
            Object userObj = body.get("username");
            Object pwObj = body.get("passwd");
            if (userObj != null)
                username = String.valueOf(userObj);
            if (pwObj != null)
                passwd = String.valueOf(pwObj);
        }
        if (username == null)
        ...
        try {
            boolean isAdmin = loginService.hasMatchAdminByUsername(username, passwd);
            boolean isReader = loginService.hasMatchReaderByUsername(username, passwd);
            if (isAdmin) {
                Admin admin = new Admin();
                Long adminId = loginService.getAdminIdByUsername(username);
                if (adminId != null)
                ...
                ReaderCard readerCard = loginService.findReaderCardByUsername(username);
    ...
    // 移除通配符映射，避免对 /index.html 等静态资源造成重定向循环
